...
from .forms import BadanieForm, BadanieListForm
...
def patient_history_list(request):
    if request.user.groups.filter(name__in=['lekarz']).exists():
        if request.method == 'POST':
            post_list = BadanieListForm(request.user, request.POST)
            print(post_list['patient'].value())
            history_list = Badanie.objects.filter(patient=post_list['patient'].value())
        else:
            history_list = BadanieListForm(request.user)
        return render(request, 'badanie/history_list.html', {'history_list': history_list, 'section': 'test'})